"""
Generate detailed report for AAPL Weekly Breakout with -2% Stock Stop Loss
Strategy: Buy ATM Call on break above prev week high
Exit: +8% profit target, -2% stock stop, or 5-day time stop
"""
import sys
...
def run_backtest_minus2_stop(symbols, years=10, capital=1500):
    """Run weekly breakout with -2% stock stop."""
    all_data = {}
    all_weekly = {}
...
def generate_report(trades, capital, final_capital, mc, wf, report_path):
    ...
    for t in trades:
        s = t['symbol']
        symbol_stats.setdefault(s, {'trades': 0, 'wins': 0, 'pnl': 0, 'profits': [], 'losses': []})
        symbol_stats[s]['trades'] += 1
        if t['pnl'] > 0:
            symbol_stats[s]['wins'] += 1
            symbol_stats[s]['profits'].append(t['pnl'])
        else:
            symbol_stats[s]['losses'].append(t['pnl'])
        symbol_stats[s]['pnl'] += t['pnl']

    md = f"""# AAPL Weekly Breakout with -2% Stock Stop Loss
...
def main():
    logger.info("="*80)
    logger.info("AAPL -2% STOCK STOP DETAILED REPORT")
    logger.info("="*80)

    logger.info("\n[1/4] Running backtest...")
    trades, capital, final_capital = run_backtest_minus2_stop(['AAPL'], years=10, capital=1500)
    ...
    report_path = Path(f'h:/QUANT TRADING/reports/NASDAQ/AAPL_MINUS2_STOP_{ts}.md')
