"""
Enhanced Backtest: AAPL Weekly Breakout with -2% Stock Stop
...
def generate_report(trades, capital, final_capital, mc, wf, report_path):
    ...
    md = f"""# Enhanced Report: AAPL Weekly Breakout with -2% Stock Stop
...
def main():
    logger.info("="*80)
    logger.info("ENHANCED BACKTEST: AAPL -2% STOCK STOP")
    logger.info("="*80)

    logger.info("\n[1/4] Running base backtest...")
    trades, capital, final_capital = run_backtest(['AAPL'], stock_stop_pct=-0.02, time_stop_days=5, years=10, capital=1500)
    ...
    report_path = Path(f'h:/QUANT TRADING/reports/NASDAQ/ENHANCED_AAPL_MINUS2_{ts}.md')
